...
if uploaded_file1 and uploaded_file2:
    try:
        ...
        # Validate columns
        if df1.shape[1] < 5 or df2.shape[1] < 5:
            st.error("One or both files do not have the required columns starting from column E.")
            st.stop()
        ...
        for row in df2.itertuples(index=False):
            ...
            if persona not in merged_df["Persona"].values:
                ...
                for idx, col in enumerate(all_personas, start=4):
                    val1 = df1.loc[df1["Persona"] == persona, col].values[0] if col in df1.columns else None
                    val2 = getattr(row, col, None)
